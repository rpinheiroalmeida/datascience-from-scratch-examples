...
# it's above the thereshold if it's not below the threshold
...
def normal_two_sided_bounds(probability, mu=0, sigma=1):
    """returns the symmetric (about the mean) bounds that contais the specified 
    probability"""
    tail_probability = (1 - probability) / 2
    # upper bound should have tail_probability above it
    upper_bound = normal_lower_bound(tail_probability, mu, sigma)

    # lower bound should have tail_probability bellow it
...
def two_sided_p_value(x, mu=0, sigma=1):
    if x >= mu:
        # if x is greater than the mean, the tail is what's greather than x
        return 2 * normal_probability_above(x, mu, sigma)
    else:
        # if x is less than the mean, the tail is what's less than x
        return 2 * normal_probability_between(x, mu, sigma)
...
def B(alpha, beta):
    """a normalizing constant so that the total probability to 1"""
    return math.gamma(alpha) * math.sqrt(beta) / math.gamma(alpha + beta)
...
if __name__ == "__main__":
    ...
    print("normal_two_sided_bounds(0.95, mu, sigma)", normal_two_sided_bounds(0.95, mu, sigma))
